...
import API from "../../services/axios"; // Import the API service to get all games
...
export default function ModeratorDashboard() {
  ...
  const handleStartNextGame = async () => {
    ...
    setError(null);
    try {
      const updatedGame = await moderatorService.updateGame(selectedNextGame, {
        status: "active",
      });
  ...
  const handleSetWinner = async () => {
    ...
    setModalError(null);
    try {
      // Use the moderatorService to set the winner
      const updatedGame = await moderatorService.setWinner(modalGameId, cardId);
        ...
        prev.map((g) =>
          g._id === modalGameId
            ? {
                ...
                status: "completed",
                winner: { cardId, prize: updatedGame.prize },
              }
            : g
        )
      );

      // Move the game to completed games if it was active
      setCompletedGames((prev) => [
        ...prev,
        { ...updatedGame, _id: modalGameId },
      ]);
      setActiveGames((prev) => prev.filter((g) => g._id !== modalGameId));
  ...
  const handleFinishGame = async (gameId) => {
    ...
    setError(null);
    try {
      // Use the moderatorService to finish the game
      const updatedGame = await moderatorService.finishGame(gameId);

      // Update the game status in the state
      setActiveGames((prev) =>
  ...
  const handleCallNumber = async () => {
    ...
    setCallNumberError(null);
    try {
      // Use the moderatorService to call the number
              ...
              <div className="space-y-6">
                <div>
                  ...
                  >
                    ...
                    {Array.from({ length: 75 }, (_, i) => i + 1).map((id) => (
